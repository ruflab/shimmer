...
class DomainDataModule(LightningDataModule):
    def __init__(
        self, val_dataset: torch.Tensor, train_dataset: torch.Tensor, batch_size: int
    ) -> None:
        super().__init__()
...
class GWDataModule(LightningDataModule):
    def __init__(
        self,
        train_datasets: dict[frozenset[str], Dataset],
        val_datasets: dict[frozenset[str], Dataset],
        batch_size: int,
    ):
        super().__init__()
    ...
    def train_dataloader(self) -> DataLoader:
        return {
            domains: DataLoader(dataset, batch_size=self.batch_size, shuffle=True)
            for domains, dataset in self.train_datasets.items()
        }

    def val_dataloader(self) -> DataLoader:
        return {
            domains: DataLoader(dataset, batch_size=self.batch_size)
            for domains, dataset in self.val_datasets.items()
        }
...
def prepare_datasets():
    ...
    train_dataset = DomainDataset(
        {"image_latents": train_images, "caption_embeddings": train_captions}
    )
    val_dataset = DomainDataset(
        {"image_latents": val_images, "caption_embeddings": val_captions}
    )

    # Create data modules
    batch_size = 64
    gw_train_data_module = GWDataModule(
        {frozenset(["image_latents", "caption_embeddings"]): train_dataset},
        batch_size=batch_size,
    )
    gw_val_data_module = GWDataModule(
        {frozenset(["image_latents", "caption_embeddings"]): val_dataset},
        batch_size=batch_size,
    )
...
if __name__ == "__main__":
    ...
    # Use `train_data_module` and `val_data_module`
