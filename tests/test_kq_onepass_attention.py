...
def test_single_domain():
    ...
    assert torch.allclose(attention_scores["v_latents"], expected_scores), "Attention scores for single domain should be all 1s"
...
def test_multiple_domains_sumis1():
    ...
    scores_sum = sum(attention_scores[domain] for domain in multiple_domain_input.keys())
    expected_sum = torch.ones(batch_size, 1)
    
    assert torch.allclose(scores_sum, expected_sum), "Sum of attention scores across domains should be 1"
